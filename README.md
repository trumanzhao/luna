...
luna的目的,早期是一个lua的C++绑定工具,现在的目的是实现一个基于lua分布式服务器框架.
主要特性:
...
注意这个实现是基于多进程的设计,并且在远程调用(RPC)中也没实现返回值.

多进程设计主要出于两个原因:
- 容灾性,设计希望集群任何服务进程都是可以宕机后恢复的(虽然lua足够稳定,但实际中难免会接入一些第三方的SDK之类的).
- 扩展性,希望实际运维时,在无需调整集群拓扑的情况下,通过增减机器方便的实现扩容缩容.

RPC为什么没有实现返回值呢?
这主要是为了简化设计,返回值完全可以用对端回调的RPC来实现.
如果一定要把异步过程写得看起来像同步过程一样,那不可避免的要引入协程来实现.
在大规模项目中,这种写法容易让新手埋下错误,如协程栈上对象有效性等问题.
在本实现中,倾向于'如果事情本身是异步的,那就让它看起来显然是异步的'.
